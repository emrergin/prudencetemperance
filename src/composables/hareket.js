function hareket(e,asama,bitis,secim,totalRevenue,totalLoss){  
    ...
    if (asama.value!=="baslangic"){return}
    ...
    function siradakiAnimasyon(){
        ...
        let elemBelow = document.elementFromPoint(rect.left-5,(rect.top+rect.bottom)/2);
        let zar= Math.floor(Math.random() * 2)+1;

        if (elemBelow.closest(`.kucukBoru`)){
            zar===1? Sol2PR() : Sag2PR();
            zar===1? etiketBoya(elemBelow.closest(`.kucukBoru`),`sol`,700) : etiketBoya(elemBelow.closest(`.kucukBoru`),`sag`,700);
            return false;
        }
        if (elemBelow.closest(`#buyukBoru`)){            
            zar===1? Sol1() : Sag1();
            let etiketler=document.getElementById(`buyukEtiketler`);
            zar===1? etiketBoya(etiketler,`sol`,1100) : etiketBoya(etiketler,`sag`,1100);
        ...
        for (let etiket of collection) {
            +etiket.textContent>0? totalRevenue.value+=+etiket.textContent : totalLoss.value+=-etiket.textContent;
        }    
    }    

    function etiketBoya(obje,yon,time){
        setTimeout(()=>{obje.querySelector(`.${yon}etiket`).classList.add(`yaklasilmis`)},time);
    }
}
